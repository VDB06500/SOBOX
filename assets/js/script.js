...
class App {
    constructor() {
        ...
        // Initialisation de la détection de connexion
    ...
    initConnectionMonitoring() {
        ...
        window.addEventListener("online", () => {
            this.updateConnectionStatus();
            this.showModal("Connexion rétablie", "Vous êtes de nouveau connecté à Internet", "success");
            // this.wifi.style.cssText = 'width:20px; display: block';
            this.pasdewifi.style.cssText = 'display:none';
        });

        window.addEventListener("offline", () => {
            this.updateConnectionStatus();
            this.showModal("Hors ligne", "Votre connexion Internet a été perdue", "warning");
            // this.wifi.style.cssText = 'display:none';
            this.pasdewifi.style.cssText = 'width:25px; display: block';
        });
    ...
    updateConnectionStatus() {
        ...
        // if (isOnline) {
        //     btnConnexion.style.cssText = 'color: #F45A2A';
        // } else {
        //     btnConnexion.style.cssText = 'color: #000';
        // }
    ...
    initializeDOMReferences() {
        ...
        document.querySelectorAll('.nav-item').forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                if (pageId !== this.currentPage) {
                    if ((pageId == 'qrcode-page') && !this.updateConnectionStatus()) {
                        this.showModal(
                            "Hors connexion",
                            "Pour partager la galerie photos, une connexion Internet est nécessaire. Veuillez vérifier votre connexion et réessayer.",
                            "error"
                        );
                        return;
                    }
        ...
        this.qrcode_code_evt = document.getElementById('qrcode_code_evt');
        ...
        this.pasdewifi = document.getElementById('pasdewifi');
        ...
        this.btn_favori = document.getElementById('btn_favori');
        this.btn_partage = document.getElementById('btn_partage');
        this.btn_galerie = document.getElementById('btn_galerie');
        ...
        document.addEventListener("visibilitychange", () => {
            if (document.visibilityState === "visible") {
                alert('Test'); // Fonction de rafraîchissement
            }
        });
    }
    ...
    async loadEvents() {
        ...
        // 1. Ré-Initialisation de la liste des événements
        console.log('// 1. Ré-Initialisation de la liste des événements');
        this.eventList.innerHTML = '';

        // 2. Ré-initialisation des galeries photos
        console.log('// 2. Ré-initialisation des galeries photos');
        this.galleryFavori.innerHTML = '';
        this.gallery.innerHTML = '';

        console.log('// 3. Affichage des événements.');

        // Si pas d'événement, on affiche une illustration
        if (this.tabListeEvents.length === 0) {
            console.log('// 3.1 Affichage message : Aucun événement enregistré.');
            this.eventList.innerHTML = '<img src="assets/images/noEvents.png" style="width:80%"></img> <p>Aucun événement enregistré.</p>';
        }
        else {
            console.log('// 3.2 Génération de la liste des événements.');
            for (const event of this.tabListeEvents) {
                ...
                if (joursRestants <= 0) {
                            ...
                            <div class="event-details">
                                <div class="event-badges">
                                    <span class="badge nbPhotos-badge">
                                         <img style="width:13px" src="assets/images/btn_galerie-dimages_on.png" alt=""> ${nbPhotos || 0}
                                    </span>
                                   
                                </div>
                                <div class="event-info">
                                    <div class="event-date">
                                        <img style="width:13px" src="assets/images/btn_calendrier.png" alt="">
                                        <p>${event.dateDebut}</p>
                                    </div>
                                    <div class="event-location">
                                        <img style="width:13px" src="assets/images/btn_location.png" alt="">
                                        <p>${event.lieu}</p>
                                    </div>
                                </div>
                            </div>
                    ...
                    eventCard.addEventListener('click', () => {
                        ...
                        console.log('// 2.2.3. Détermination du statut de l\'événement : ' + statut);
                        ...
                        console.log('// 6. Appel : initPhotobooth');
                    ...
                    <div class="event-details">
                        <div class="event-badges">
                            <span class="badge nbPhotos-badge">
                                <img style="width:13px" src="assets/images/btn_galerie-dimages_on.png" alt=""> PROCHAINEMENT
                            </span>
                        </div>
                        <div class="event-info">
                            <div class="event-date">
    ...
    async navigateTo(pageId) {
        ...
        // 1. Arrêt de la synchronisation
        await this.stopImageWatch();

        // 2. Masquer toutes les pages
        document.querySelectorAll('.page').forEach(page => {
            page.classList.remove('active');
        });

        // 3. Afficher la page en paramètre
        const activePage = document.getElementById(pageId);
        if (activePage) {
            activePage.classList.add('active');
        }

        // 4. Gérer la visibilité de la navigation
        ...
        // 5. Page : Liste des événements
        if (pageId === 'events-page') this.initEvents();

        // 6. Page : Rejoindre un événement
        ...
        // Page : QR-CODE
        if (pageId === 'qrcode-page') {
            if (this.updateConnectionStatus()) {
                this.initQrcode(pageId);
            }
            else {
                this.showModal(
                    "Hors connexion",
    ...
    async initPhotobooth(pageId) {
        ...
        // Mise à jour pico des menus
        this.btn_favori.src = 'assets/images/btn_favori_off.png';
        this.btn_partage.src = 'assets/images/btn_partager_off.png';
        this.btn_galerie.src = 'assets/images/btn_galerie-dimages_on.png';
    ...
    async initFavorites(pageId) {
        ...
        // Mise à jour des picto des menus
        this.btn_favori.src = 'assets/images/btn_favori_on.png';
        this.btn_partage.src = 'assets/images/btn_partager_off.png';
        this.btn_galerie.src = 'assets/images/btn_galerie-dimages_off.png';
    ...
    async toggleFavorite(image) {
        try {
            ...
            request.onsuccess = () => {
                ...
                if (this.nbFavori > 0) {
                    // Désactivation du message "Pas de favori"
                    this.pasFavori.style.display = 'none';
                    this.lib_nb_favori.textContent = this.nbFavori === 1 ? '1 favori' : this.nbFavori + ' favoris';
                } else {
                    // D&Activation du message "Pas de favori"
                    this.pasFavori.style.display = 'block';
                    this.lib_nb_favori.textContent = 'Aucun favori';
                }
    ...
    updateUIAfterFavoriteToggle(imageUrl, isFavorite) {
        ...
        document.querySelectorAll(`.gallery-item [data-url="${imageUrl}"] .heart-btn img`).forEach(img => {
            alert('x');
            img.src = isFavorite ? 'assets/images/btn_heart_on.png' : 'assets/images/btn_heart_off.png';
            // Si vous voulez aussi changer la couleur du bouton comme dans l'exemple
            //img.parentElement.style.background = isFavorite ? 'rgba(255,0,0,0.3)' : 'rgba(0,0,0,0.5)';
        });
    ...
    async initQrcode(pageId) {
        ...
        // 1. Retirer active de tous les éléments
        document.querySelectorAll('.nav-item').forEach(navItem => {
            navItem.classList.remove('active');
        });

        // 2. Activation de la page "QRCODE"
        const activeNavItem = document.querySelector(`.nav-item[data-page="${pageId}"]`);
        if (activeNavItem) {
            activeNavItem.classList.add('active');
        }
        ...
        // Mise à jour pico des menus
        this.btn_favori.src = 'assets/images/btn_favori_off.png';
        this.btn_partage.src = 'assets/images/btn_partager_on.png';
        this.btn_galerie.src = 'assets/images/btn_galerie-dimages_off.png';

        // 3. Génération du QRCode
        await this.generateQR();

        // 4.  Lecture du code événement
        // Lecture du nom de l'événement
        const eventData = JSON.parse(localStorage.getItem("Evenement_En_Cours"))
        this.qrcode_code_evt.innerHTML = "Saisissez le code : " + eventData.code + " pour accéder aux photos";
        ...
        this.partageAccesSms.addEventListener('click', async () => {
            ...
            if (this.updateConnectionStatus() && eventData.statut < 2) {
                ...
                this.showModal(
                    "Hors connexion",
                    "Pour partager le lien de la galerie photos, une connexion Internet est nécessaire. Veuillez vérifier votre connexion et réessayer.",
                    "error"
                );
            }
    ...
    async initParam() {
        // 1. Désactivation du menu Footer
        this.toggleNavBar(false);

        // 2. Activation du bouton "Retour"
        this.btnRetour.style.display = 'block';

        // 3. Activation du bouton "Close"
        this.btnClose.style.display = 'none';

        // 4. Gestion de l'activation de l'accès adm 
        ...
        // 5. Désactivation du logo SoBox
        this.logoSobox.style.display = 'none';

        // 6. Désactivation du bouton "Paramètres"
        this.btnParam.style.display = 'none';

        // 7. Mise à jour du titre de la photo
    ...
    async initZoom() {
        ...
        this.titreHeader.textContent = 'Visualisation';
    ...
    async initEvents() {
        ...
        this.btnParam.style.display = 'block';  // Activation du bouton "Paramètres"
    ...
    async rejoindreEvenement() {
        ...
        this.toggleNavBar(false);  // Désactivation du menu Footer
        this.titreHeader.textContent = 'Rejoindre un événement'; // Titre de la fenêtre
        this.btnRetour.style.display = 'flex'; // Activation du bouton retour
        this.logoSobox.style.display = 'none'; // Désactivation du logo
        this.btnParam.style.display = 'none'; // Activation du bouton "Paramètres"
        this.eventCodeInput.value = ''; // Ré initialisation de l'INPUT

        // 2. On se position sur l'INPUT
        this.eventCodeInput.focus();

        // 3. Filtrage des données : Lettres et chiffres uniquement + Forcing majuscule
        eventCodeInput.addEventListener('input', function () {
            // 4. Sauvegarde la position du curseur
            const start = this.selectionStart;
            const end = this.selectionEnd;

            // 5. Filtre et limite à 10 caractères
            this.value = this.value
                .toUpperCase()
                .replace(/[^A-Z0-9]/g, '')
                .substring(0, 10); // ← Limite à 10 caractères

            // 6. Restaure la position du curseur
            if (start > 10) start = 10;
            if (end > 10) end = 10;
            this.setSelectionRange(start, end);

            // 7. Feedback visuel (optionnel)
    ...
    async handleEventCodeSubmission(code) {
        try {
            ...
            if (this.updateConnectionStatus() && eventData.statut < 2) {
                ...
                this.showModal(
                    "Hors connexion",
                    "Pour rejoindre un événement, une connexion Internet est nécessaire. Veuillez vérifier votre connexion et réessayer.",
                    "error"
                );
            }
    ...
    async addEventToDB(event) {
        ...
        console.log('---------------------------------------------');
        console.log('Appel addEventToDB()');
        console.log('--> Paramètre : ' + event);
        console.log('---------------------------------------------');

        // 1. Vérifier que `events` est disponible
        console.log('1. Vérifier que `events` est disponible')
        ...
        if (!db) {
            ...
            // 1.1. Base de données non initialisée
            console.log('1.1. Base de données non initialisée')
        ...
        return new Promise((resolve, reject) => {
            ...
            // 2. Connexion à la BDD locale "events"
            console.log('2. Connexion à la BDD locale "events"');
            ...
            checkRequest.onsuccess = () => {
                if (checkRequest.result) {
                    ...
                    // 2.1. L'enregistrement existe
                    console.log('2.1. L\'enregistrement existe');
                ...
                // 2.2. Ajout de l'événement
                console.log('2.2. Ajout de l\'événement');

                const addRequest = store.add(event);

                addRequest.onsuccess = () => {
                    // 2.3 Événement ajouté
                    console.log('2.3 Événement ajouté : ', event);
                ...
                addRequest.onerror = (e) => {
                    // 2.4 Erreur IndexedDB
                    console.error('2.4 Erreur IndexedDB : ', e.target.error);
            ...
            checkRequest.onerror = (e) => {
                // 2.5 Erreur vérification existance
    ...
    //  Appel API : Recherche et ajout du code événement
    async fetchEvent(eventCode) {

        console.log('---------------------------------------------');
        console.log('Appel fetchEvent()');
        console.log('---------------------------------------------');

        // 1. Génération du lien API
        console.log('1. Génération du lien API');
        const apiUrl = `${this.urlProjet}/assets/php/get_events.php?password=${encodeURIComponent(eventCode)}`;

        // 2. Appel de l\'API
        console.log('2. Appel de l\'API : ' + apiUrl);

        let response;
        let result;

        try {
            // 2.1. Réponse de l\'API');
            console.log('2.1. Réponse de l\'API');
            ...
            console.log('2.1. Statut HTTP:', response.status);
            console.log('2.1. Headers:', Object.fromEntries(response.headers.entries()));
            ...
            // 2.2  Erreur de l\'API';
            console.log('2.2. Erreur de l\'API');
        ...
        if (response.status === 301 || response.status === 302) {
            console.log('3. Gestion des redirections : ' + response.status);
        ...
        if (!response.ok) {
            console.log('3. Erreurs HTTP (4xx/5xx)');
            return {
                status: "error",
                message: 'Erreur technique.Impossible d\'ajouter l\'événement souhaité.',
                data: null
            };
        }

        // 5. Erreur métier (ex: mot de passe incorrect)
        if (result.status != "succes") {
            console.log('5. Erreur métier');
            return result; // On retourne tel quel le message d'erreur de l'API
        }

        // 6. Succès → Mise à jour de la base locale
        console.log('6. Succès → Mise à jour de la base locale');
    ...
    createImageCard(image, index) {
        ...
        heartBtn.innerHTML = '<img style="width:13px" src="assets/images/btn_heart_off.png" alt="">'; // Cœur vide par défaut
        ...
        if (image.favori) {
            heartBtn.innerHTML = '<img style="width:13px" src="assets/images/btn_heart_on.png" alt="">'
        }
        // Gestion du clic sur le cœur
        heartBtn.addEventListener('click', (e) => {
            e.stopPropagation(); // Empêche le déclenchement du clic sur la carte
            heartBtn.innerHTML = heartBtn.innerHTML.includes('btn_heart_off.png')
                ? "<img style='width:13px' src='assets/images/btn_heart_on.png' alt=''>"
                : "<img style='width:13px' src='assets/images/btn_heart_off.png' alt=''>";
    ...
    async startImageWatch() {
        ...
        if (this.updateConnectionStatus() && eventData.statut < 2) {
            ...
            this.showModal("Vous êtes hors connexion", "Pas de synchronisation des images", "warning");
        }
